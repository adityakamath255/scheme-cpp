...
const shareBtn = /** @type {HTMLButtonElement} */ (document.getElementById("share"));
const timing = /** @type {HTMLElement} */ (document.getElementById("timing"));
const gutter = /** @type {HTMLElement} */ (document.getElementById("gutter"));
...
function fromHash() {
  ...
  try { return b64.dec(decodeURIComponent(location.hash.slice(1))); }
  catch { return null; }
}
...
function run() {
  ...
  try {
    while (buffer.trim()) {
      ...
      if (r.kind === "incomplete") { append("unexpected end of input", "err"); return; }
      if (r.kind === "exhausted") return;
      if (r.value !== undefined) append(r.value, "res");
      buffer = r.rest;
    }
  } catch (ex) {
...
function disposeSession() {
  if (session) { try { session.delete(); } catch {} }
}
...
async function share() {
  ...
  catch { shareBtn.textContent = "Link in URL"; }
